...
- See a weekly summary of your tracked time with `ttcli --week` (or `--week YYYY-MM-DD` for a specific week, where YYYY-MM-DD is a date within that week). Note that you can change which day the week starts on with the `--week-start` option (default is Saturday).
